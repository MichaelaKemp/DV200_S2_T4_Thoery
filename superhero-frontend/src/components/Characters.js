...
const Characters = () => {
  ...
  const fetchAllHeroes = async () => {
    try {
      const result = await axios.get('http://localhost:3001/api/heroes'); // Adjust API endpoint if needed
  ...
  const filterHeroes = async (searchQuery, alignment) => {
    try {
      ...
      if (searchQuery) {
        const result = await axios.get(`http://localhost:3001/api/superhero?name=${searchQuery}`);
        ...
        const result = await axios.get('http://localhost:3001/api/heroes');
        filteredHeroes = result.data;
      }
